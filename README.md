...
| **快捷分析** | 选择对象后输入文字、可选上传截图（≤10MB，点击或拖拽）；可选意图标签。点击 **开始分析** 调用智谱对话补全，解析 JSON 后跳转 **分析结果**（共情开场、建议话术、情境分析）。底部主按钮区域位置固定，突出操作按钮。 |
| **分析结果** | 展示本轮输入、截图与模型输出；页顶 **返回**；侧栏入口始终可回达。 |
| **深度档案** | 沟通风格、驱动力、雷区与甜点区等洞察；页顶 **返回**；侧栏入口始终可回达。 |
